...
export default class TenantCache {
   ...
   */
  async get<T>(tenantName: string, key: NodeCache.Key, storeFn?: ()=>Promise<T> ) {
   ...
   */
  flushAll() {
      Object.keys(this.tenantCaches).forEach(
          k => {
  ...
  public on(eventName, eventFn: IEventFunction) {
    ...
    Object.values(this.tenantCaches).forEach(
        c => c.on(eventName, eventFn)
    )
  }
